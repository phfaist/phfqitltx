...
%\usepackage[title=default,pkgset=extended]{phfnote}
%\usepackage[title=defaultv1,pkgset=extended]{phfnote}
%\usepackage[title=small,pkgset=extended]{phfnote}
%\usepackage[title=smallv1,pkgset=extended]{phfnote}
%\usepackage[title=pretty,pkgset=extended]{phfnote}
%\usepackage[title=pretty2,pkgset=extended]{phfnote}
%\usepackage[title=article,pkgset=extended]{phfnote}
%\usepackage[title=articlev1,pkgset=extended]{phfnote}
...
%\usepackage[preset=reset,pkgset=extended,title=small,secfmt={section},spacingdefs,fontdefs,pagegeomdefs,pagegeom=xwide,bibliographydefs,abstract={wide,small,it}]{phfnote} \usepackage{hyperref}\makeatletter\let\email\url\makeatother\phfnotePdfLinkColor{red}
...
%\usepackage[preset=sfssnote,pkgset=extended,pagegeom=xwide]{phfnote}
%\usepackage[title=default,pkgset=extended,secfmt={section},hyperrefdefs=true]{phfnote}
%\usepackage[title=default,pkgset=extended,secfmt={section},hyperrefdefs=defer]{phfnote}\usepackage[unicode=false,debug]{hyperref}
%\usepackage[title=default,pkgset=extended,secfmt={section},hyperrefdefs={noeqref,defer}]{phfnote}\usepackage[unicode=false,debug]{hyperref}
%\usepackage[title=default,pkgset=extended,secfmt={section},hyperrefdefs=noload]{phfnote} \def\href#1#2{#2}
...
\iffalse%\iftrue  % swap true/false with META-T
  \def\notetitleprettylsiderulewidth{10pt}
  \def\notetitleprettylsidespacewidth{20pt}
  \def\notetitleprettyrsiderulewidth{.4pt}
  \def\notetitleprettyrsidespacewidth{29.6pt}
  \def\notetitleprettytopspace{10pt}
  \def\notetitleprettybottomspace{20pt}
  \def\notetitleprettytophrulewidth{1pt}
  \def\notetitleprettybottomhrulewidth{2pt}

  %  \def\notetitlebeginrender{\vbox\bgroup\hsize=6cm}\def\notetitleendrender{\egroup}
  %  \notetitleusempfootnotesfalse

  \definecolor{notetitleprettylsiderulecolor}{RGB}{0,68,126}
  \colorlet{notetitleprettyrsiderulecolor}{blue}
  \colorlet{notetitleprettytophrulecolor}{red}
  \colorlet{notetitleprettybottomhrulecolor}{red!35!green}
  ...
  % \def\notetitleprettyboxtophrule{%
  %   {\color{red!35!green}\hrule height 1ex}}
\fi

\iffalse%\iftrue
  \def\notetitlewidth{1.2\textwidth}
\fi


%\renewcommand\notetitleauthorfont[1]{\bfseries #1}

\iftrue%\iffalse  % swap true/false with META-T
...
%\def\notetitleinnervsep{0pt}
%\renewcommand\notetitlesmallauthordatesep{\hspace*{.5em}\cdot\hspace*{.5em}}
%\csdef{phfnote@fmt@titlefootnotes}{\itshape}
%\title{A title goes like this}
%\title{A two-part title can be set\par With a \string\par\ command}
\title{A very long title would go like this: Lorem ipsum dolor amet stuff with excelsior borat et solus}
%\author{Mister Me}
\author{Mister Me\thanks{Thanks to someone.}}
%\author{Author One$^{1}$ and Second Author$^2$\\[1ex]
% \emph{$^1$First institution}\\
% \emph{$^2$Another institution}}
%\date{}
\date{\today}
...
\begin{abstract}
  ...
  \iffalse%\iftrue % test longer/shorter abstract
  Using Landauer's Principle, we reduce the characterization of work cost or
  extraction to the consideration of absorbtion or generation during noisy
  operations of randomness stored in ancillas, similar to the notion of
  thermomajorization introduced by Horodecki et al.~[arXiv:1111.3834v1].  This
  framework provides a suitable tool to find a lower bound to the work cost of
  the erasure of a system possessing quantum correlations with a memory,
  expressed as the min-entropy of the system conditioned on the memory. This
  bound is complementary to the upper bound presented in~[del Rio et al.,
  Nature, 2011].
  \fi
...
\url{http://dx.doi.org/10.1038/ncomms8669}.  An email would be \email{someone-a@some_place.com}

Landauer's Principle~\cite{Landauer1961_5392446Erasure}, and more generally the
relation between the second law of thermodynamics and information theory, has
received tons of attention in the past decades.  Studies have notably focused on
heat generated by computation~\cite{Bennett1982IJTP_ThermodynOfComp}, the
exorcism of Maxwell's demon via information theory (see
eg.~\cite{Bennett2003_NotesLP}), and generalizations to quantum settings such as
characterization of entanglement through thermodynamical
considerations~\cite{Oppenheim2002PRL_thermodynamical}, or the determination of
the single shot work cost of the erasure of a quantum system with quantum side
information~\cite{delRio2011Nature}.
